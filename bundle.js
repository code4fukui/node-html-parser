...
var CommentNode = class _CommentNode extends Node {
  ...
  constructor(rawText, parentNode = null, range, rawTagName = "!--") {
    super(parentNode, range), this.rawText = rawText, this.rawTagName = rawTagName, /**
...
var getCodePoint = (
  ...
  String.prototype.codePointAt != null ? (str, index) => str.codePointAt(index) : (c, index) => (c.charCodeAt(index) & 64512) === 55296 ? (c.charCodeAt(index) - 55296) * 1024 + c.charCodeAt(index + 1) - 56320 + 65536 : c.charCodeAt(index)
);
...
var foreignElements = /* @__PURE__ */ new Set([
  "svg",
  "math"
]);
...
function existsOne(test, nodes) {
  return (Array.isArray(nodes) ? nodes : [
    nodes
  ]).some((node) => isTag2(node) && test(node) || hasChildren(node) && existsOne(test, node.children));
}
...
var import_boolbase5 = __toESM(require_boolbase());
...
var import_boolbase2 = __toESM(require_boolbase());
function compile(parsed) {
  ...
  if (a === 1) return b < 0 ? import_boolbase2.default.trueFunc : (index) => index >= b;
...
var import_boolbase3 = __toESM(require_boolbase());
...
var filters = {
  ...
  "nth-child"(next, rule, { adapter, equals }) {
    ...
    if (func === import_boolbase3.default.trueFunc) return getChildFunc(next, adapter);
...
var import_boolbase4 = __toESM(require_boolbase());
var PLACEHOLDER_ELEMENT = {};
function ensureIsTag(next, adapter) {
  if (next === import_boolbase4.default.falseFunc) return import_boolbase4.default.falseFunc;
...
var subselects = {
  ...
  has(next, subselect, options, _context, compileToken2) {
    ...
    if (compiled === import_boolbase4.default.falseFunc) return import_boolbase4.default.falseFunc;
...
function getAttributeValue2(elem, name) {
  return isTag3(elem) ? elem.getAttribute(name) : void 0;
}
...
function existsOne2(test, elems) {
  return elems.some((elem) => {
    return isTag3(elem) ? test(elem) || existsOne2(test, getChildren2(elem)) : false;
  });
}
...
function hasAttrib2(elem, name) {
  return getAttributeValue2(elem, name) !== void 0;
}
function findOne2(test, elems) {
  ...
  for (let i = 0, l = elems?.length; i < l && !elem; i++) {
    ...
    if (test(el)) {
      ...
      const childs = getChildren2(el);
      if (childs && childs.length > 0) {
        elem = findOne2(test, childs);
      }
    }
  }
  return elem;
...
function findAll2(test, nodes) {
  ...
  for (let i = 0, j = nodes.length; i < j; i++) {
    if (!isTag3(nodes[i])) continue;
    if (test(nodes[i])) result.push(nodes[i]);
    const childs = getChildren2(nodes[i]);
    if (childs) result = result.concat(findAll2(test, childs));
  }
  return result;
}
var matcher_default = {
...
var TextNode = class _TextNode extends Node {
  ...
  * @type {Number}
  */
  nodeType = type_default.TEXT_NODE;
  rawTagName = "";
  _rawText;
  _trimmedRawText;
  _trimmedText;
...
var HTMLElement = class _HTMLElement extends Node {
  ...
  */
  constructor(tagName, keyAttrs, rawAttrs = "", parentNode = null, range, voidTag = new VoidTag(), _parseOptions = {}) {
    super(parentNode, range), this.rawAttrs = rawAttrs, this.voidTag = voidTag, this.nodeType = type_default.ELEMENT_NODE;
  ...
  */
  getElementsByTagName(tagName) {
    ...
    while (index !== void 0) {
      ...
      if (child.nodeType === type_default.ELEMENT_NODE) {
...
function base_parse(data, options = {}) {
  ...
  while (match = kMarkupPattern.exec(data)) {
    let { 0: matchText, 1: leadingSlash, 2: tagName, 3: attributes2, 4: closingSlash } = match;
...
function parse4(data, options = {}) {
  return parse3(data, options);
}
parse4.parse = parse3;
parse4.HTMLElement = HTMLElement;
parse4.CommentNode = CommentNode;
parse4.valid = valid;
parse4.Node = Node;
parse4.TextNode = TextNode;
parse4.NodeType = type_default;
export {
  CommentNode,
  HTMLElement,
  Node,
  type_default as NodeType,
  TextNode,
  parse4 as default,
  parse4 as parse,
  valid
